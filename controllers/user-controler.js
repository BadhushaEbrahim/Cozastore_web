...
module.exports = {
    ...
    PlaceOrder: async (req, res) => {
        ...
        if (verifyCoupon.couponId == req.body.couponcode) {
            ...
            const discountAmount = Math.floor((totalPrice * parseInt(verifyCoupon.couponPercentage)) / 100);
    ...
    getUserProfile: async (req, res) => {
        const user = req.session.user;
        if (user) {
            const allAddress = await userHelper.getShippingAddress(req.session.user._id)
            let cartCount = await userHelper.getCartCount(req.session.user._id)
            let wishlistCount = await userHelper.getWishlistCount(req.session.user._id)
            const categories = await userHelper.getAllcategories()
    ...
    postCouponApply: async (req, res) => {
        ...
        if (req.body.coupon == '') {
            ...
            const couponres = await userHelper.applyCoupon(req.body, user, date, totalAmount)
            if (couponres.verify) {

                const discountAmount = (totalAmount * parseInt(couponres.couponData.couponPercentage)) / 100;
    ...
    displayWishList: async (req, res) => {
        ...
        if (user) {
            ...
            const products = await userHelper.getWishListProducts(req.session.user._id)
            let totalValue = 0;
            if (products.length > 0) {
                totalValue = await userHelper.getTotalAmount(req.session.user._id)
            }
